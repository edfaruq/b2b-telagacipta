...
export async function GET() {
  ...
  try {
    ...
    const autoShipReady = canAutoCreateShipment();
    ...
    const shipments = (
      ...
      const biteshipCourier = mapExpeditionToBiteshipCourier(expedition);
      ...
      return {
        ...
        biteshipOrderId: row.biteship_order_id,
        canUseBiteship:
          autoShipReady && row.status_pengiriman === "diproses" && Boolean(biteshipCourier),
      };
    });

    return NextResponse.json({
      shipments,
      biteshipConfigured: autoShipReady,
      shippingMock: isMockShippingEnabled(),
    });
  } catch (err) {
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      viaBiteship: result.viaBiteship,
